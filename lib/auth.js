...
const authCookieName = 'astra_session';
const SALT_ROUNDS = 10;

function getAuthSecret() {
  return process.env.SITE_PASSWORD || process.env.FRIEND_USERS || 'astra-local';
}
...
function createAuthToken(username) {
  ...
  const signature = crypto
    ...
    .update(`astra-browser-access:${safeUsername}`)
    .digest('hex');
...
function hasSitePassword() {
  return Boolean(process.env.SITE_PASSWORD || process.env.FRIEND_USERS);
}
