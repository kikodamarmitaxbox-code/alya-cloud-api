...
function buildPrompt(task, contextFiles, extraContext = '', history = [], projectId = 'main') {
  ...
  const conversation = Array.isArray(history)
    ? history
      ...
      .slice(-10)
      .map((item) => `${item.role === 'user' ? 'PEDRO' : 'SOFIA'}: ${String(item.content || '').slice(0, 1600)}`)
      .join('\n')
    : '';
...
async function runSafeCommand(command, projectId = 'main', onProgress = null) {
  ...
  return new Promise((resolve) => {
    ...
    child.on('close', (code) => {
      finish(code === 0 && !timedOut, timedOut ? '\nTempo máximo de 60 segundos atingido.' : '');
    });
...
async function applyCodePlan(planId, onProgress = null) {
  ...
  try {
    ...
    for (const action of plan.actions) {
      ...
      report(`Atualizando arquivo: ${action.path}`);
      const filePath = resolveSafePath(action.path, projectId);
