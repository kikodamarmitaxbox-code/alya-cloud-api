...
async function createCodePlan(task, contextFiles = [], history = [], options = {}) {
  ...
  try {
    ...
    response = await askModel(`${prompt}

CORREÇÃO DE FORMATO:
Sua resposta anterior não pôde ser lida. Responda agora SOMENTE com um objeto JSON curto:
{"summary":"o que será feito","actions":[{"type":"command","command":"npm run verify"}]}
Use "done" quando nenhuma ação for necessária. Não use markdown, comentários ou texto fora do JSON.`);
    plan = parsePlan(response.text);
  }
