...
function listProjects() {
  const projects = [{
    ...
    description: 'Projeto principal da Sofia'
  }];
...
function createProject(name) {
  ...
  fs.writeFileSync(
    ...
    `# ${displayName}\n\nProjeto criado pela Code Sofia.\n`,
    'utf8'
  );
...
function sanitizeCommandOutput(output) {
  return String(output || '')
    ...
    .replace(/AQ\.[A-Za-z0-9_-]{16,}/g, '[chave oculta]')
    .replace(/AIza[A-Za-z0-9_-]{16,}/g, '[chave oculta]')
    .replace(/(token|password|secret|api[_-]?key)\s*[=:]\s*[^\s"'`]+/gi, '$1=[oculto]');
}
...
function commandTimeout() {
  const configured = Number(process.env.ALYA_CODE_COMMAND_TIMEOUT_MS || 180000);
  return Math.min(600000, Math.max(15000, Number.isFinite(configured) ? configured : 180000));
}
...
function codeSystemPrompt() {
  return `Você é Code Sofia, uma agente profissional de programação dentro de um painel visual.
Seu trabalho é concluir pedidos de programação de ponta a ponta: entender, investigar, editar, testar, corrigir falhas e validar o resultado real.
...
- Quando Pedro pedir uma mudança, nunca responda apenas "done" nem prometa que vai alterar sem incluir ações. Use "read" até entender o código, depois "patch", "write", "move", "delete" e "command" para concluir e verificar a mudança.
- Entregue uma solução funcional e completa, incluindo todos os arquivos relacionados. Não pare depois de alterar apenas uma parte quando outras forem necessárias.
- Depois de cada mudança, inclua testes ou verificações adequadas. Se um teste real falhar, use o resultado na próxima etapa para corrigir a causa.
- Você pode criar arquivos e pastas por meio de "write", renomear arquivos com "move" e apagar arquivos com "delete". Essas ações sempre serão mostradas ao Pedro e terão backup antes de serem executadas.
...
- Se Pedro pedir somente análise e proibir alterações, não use ações "patch", "write", "move" ou "delete". Sugira apenas comandos seguros de verificação quando eles forem necessários.
...
- Use no máximo ${MAX_PLAN_ACTIONS} ações por etapa. Quando a tarefa for maior, conclua em várias etapas supervisionadas.
...
FORMATO:
{
  ...
  "actions": [
    ...
    {"type":"move","from":"arquivo-antigo","to":"arquivo-novo"},
    {"type":"delete","path":"arquivo-obsoleto"},
  ...
  for (const candidate of [...new Set(candidates)]) {
    try {
      ...
      return {
        ...
        actions: Array.isArray(parsed.actions) ? parsed.actions.slice(0, MAX_PLAN_ACTIONS) : []
      };
    } catch {
...
function hasActionablePlan(actions) {
  return (Array.isArray(actions) ? actions : [])
    .some((action) => ['read', 'patch', 'write', 'move', 'delete', 'command']
...
function getProjectKnowledgeContext(task, contextFiles = [], projectId = 'main') {
  const files = getRelevantProjectFiles(task, contextFiles, projectId, 120);
  const entries = retrieveProjectKnowledge({
    query: task,
    files,
    explicitFiles: contextFiles,
    scanLimit: Number(process.env.ALYA_CODE_KNOWLEDGE_SCAN_LIMIT || 80),
    limit: Number(process.env.ALYA_CODE_KNOWLEDGE_FILES || 4),
    readFile(file) {
      return readProjectFile(file, 16000, projectId).content;
    }
  });
  return formatProjectKnowledge(
    entries,
    Number(process.env.ALYA_CODE_KNOWLEDGE_MAX_CHARS || 12000)
  );
}
...
async function askModel(prompt, options = {}) {
  ...
  if (errors.length) logger.warn(`Code Sofia sem provedor disponível: ${errors.join(' • ')}`);
  throw new Error(
    errors.length
      ? 'Os modelos de programação não responderam agora. A Code Sofia tentou as opções configuradas; aguarde um pouco e tente novamente.'
...
function buildPrompt(task, contextFiles, extraContext = '', history = [], projectId = 'main') {
  const sections = [];
  for (const requestedPath of [...new Set(contextFiles || [])].slice(0, 12)) {
    try {
  ...
  const conversation = Array.isArray(history)
    ? history
      ...
      .slice(-16)
      .map((item) => `${item.role === 'user' ? 'PEDRO' : 'SOFIA'}: ${String(item.content || '').slice(0, 2200)}`)
      .join('\n')
    : '';
  const relevantFiles = getRelevantProjectFiles(task, contextFiles, projectId, 80);
  const projectKnowledge = getProjectKnowledgeContext(task, contextFiles, projectId);
  return `HISTÓRICO DA CONVERSA:\n${conversation || 'Esta é a primeira mensagem.'}

PEDIDO ATUAL DO PEDRO:\n${String(task).slice(0, 6000)}

ARQUIVOS DO PROJETO MAIS RELEVANTES:
${relevantFiles.join('\n')}

ARQUIVOS ABERTOS:
${sections.join('\n') || 'Nenhum arquivo aberto.'}

TRECHOS ENCONTRADOS AUTOMATICAMENTE PELO CONTEÚDO:
${projectKnowledge || 'Nenhum trecho adicional correspondeu ao pedido.'}
  ...
  for (const action of Array.isArray(actions) ? actions : []) {
    ...
    try {
      if (
        ...
        const normalizedPath = relativePath(filePath, selectedRoot);
        const currentContent = plannedContents.has(normalizedPath)
          ? plannedContents.get(normalizedPath)
          : fs.readFileSync(filePath, 'utf8');
        ...
        const content = currentContent.replace(action.find, action.replace);
        const previousWrite = normalized.findLast?.(
          (item) => item.type === 'write' && item.path === normalizedPath
        );
        if (previousWrite) {
          previousWrite.content = content;
        } else {
          normalized.push({
            type: 'write',
            path: normalizedPath,
        ...
        plannedContents.set(normalizedPath, content);
      } else if (type === 'write' && action.path && typeof action.content === 'string') {
        if (action.content.length > 1000000) throw new Error('Arquivo grande demais.');
        const filePath = resolveSafePath(action.path, projectId);
        const normalizedPath = relativePath(filePath, selectedRoot);
        const previousWrite = normalized.findLast?.(
          (item) => item.type === 'write' && item.path === normalizedPath
        ...
        if (previousWrite) {
          previousWrite.content = action.content;
        } else {
          normalized.push({
            type,
            path: normalizedPath,
        ...
        plannedContents.set(normalizedPath, action.content);
      } else if (type === 'move' && action.from && action.to) {
        const source = resolveSafePath(action.from, projectId);
        const destination = resolveSafePath(action.to, projectId);
        if (source === destination) throw new Error('Origem e destino são iguais.');
        if (!fs.existsSync(source) || !fs.statSync(source).isFile()) {
          throw new Error('Arquivo de origem não encontrado.');
        }
        if (fs.existsSync(destination)) {
          throw new Error('O destino do arquivo já existe.');
        }
        normalized.push({
          type,
          from: relativePath(source, selectedRoot),
          to: relativePath(destination, selectedRoot)
        });
      } else if (type === 'delete' && action.path) {
        const filePath = resolveSafePath(action.path, projectId);
        if (!fs.existsSync(filePath) || !fs.statSync(filePath).isFile()) {
          throw new Error('Arquivo para apagar não encontrado.');
        }
        normalized.push({
          type,
          path: relativePath(filePath, selectedRoot)
...
async function createCodePlan(task, contextFiles = [], history = [], options = {}) {
  ...
  if (!cleanTask) throw new Error('Conte para a Code Sofia o que você quer criar.');
...
Se ainda precisa entender o código, devolva ações "read". Caso já saiba a correção, devolva ações "patch", "write", "move" ou "delete" e comandos seguros de teste.`, { onProgress }));
  }
  if (!hasActionablePlan(plan.actions) && looksLikeChangeRequest(cleanTask)) {
  ...
  const accumulatedContextFiles = [...new Set(contextFiles || [])].slice(0, 12);
  const loadedFiles = new Set(accumulatedContextFiles);
  for (let readPass = 0; readPass < MAX_READ_PASSES; readPass += 1) {
    const requestedReads = plan.actions
      .filter((action) => String(action?.type).toLowerCase() === 'read' && action.path)
      .map((action) => String(action.path))
      .filter((requestedPath) => !loadedFiles.has(requestedPath))
      .slice(0, 6);
    if (!requestedReads.length) break;

    onProgress(`Lendo ${requestedReads.length} arquivo(s) necessário(s)...`);
    for (const requestedPath of requestedReads) {
      loadedFiles.add(requestedPath);
      accumulatedContextFiles.push(requestedPath);
    }
    const actionInstruction = looksLikeChangeRequest(cleanTask)
      ? 'Prepare agora ações concretas "patch", "write", "move", "delete" ou "command". Se ainda faltar contexto, peça somente os novos arquivos realmente necessários.'
      : 'Responda ao pedido usando os arquivos carregados. Se ainda faltar contexto, peça somente os novos arquivos realmente necessários.';
    ({ response, plan } = await askForValidPlan(buildPrompt(
      cleanTask,
      accumulatedContextFiles,
      `${extraContext}\n\nOs arquivos pedidos nas etapas anteriores já foram carregados. ${actionInstruction}`,
      history,
      projectId
    ), { onProgress }));
  }
  ...
  let actions = normalizeActions(plan.actions, projectId);
  if (!actions.length && looksLikeChangeRequest(cleanTask)) {
    plan = {
      summary: 'O modelo sugeriu ações que não passaram pelas proteções. Vou verificar o estado real do projeto antes de tentar outra correção.',
      actions: [{ type: 'command', command: 'git status' }]
    };
    actions = normalizeActions(plan.actions, projectId);
  }
  const existingSession = getAgentSession(sessionId);
  const session = existingSession || {
    id: sessionId,
    task: cleanTask,
    contextFiles: accumulatedContextFiles.slice(0, 12),
  ...
  return {
    ...
    preview: actions
      .filter((action) => ['write', 'move', 'delete'].includes(action.type))
...
function validateCommand(command) {
  ...
  const allowed = [
    /^node\s+--check\s+[\w./\\-]+$/i,
    /^node\s+--test(?:\s+[\w./\\*?-]+)*$/i,
    /^node\s+[\w./\\-]+\.[cm]?js(?:\s+[\w./\\="'-]+)*$/i,
    /^node\s+scripts[\\/]verify\.js$/i,
    /^npm\s+(test|run\s+[\w:-]+)(?:\s+--\s+[\w./\\-]+)*$/i,
    /^npm\s+ci(?:\s+--(?:omit=dev|ignore-scripts|no-audit|no-fund))*$/i,
    /^npm\s+audit(?:\s+--omit=dev)?$/i,
    /^npm\s+install(?:\s+--(?:save|save-dev|save-exact|no-audit|no-fund|ignore-scripts))*(?:\s+(?:@[\w.-]+\/)?[\w.-]+)*(?:\s+--(?:save|save-dev|save-exact|no-audit|no-fund|ignore-scripts))*$/i,
    /^npx\s+--no-install\s+(eslint|prettier|tsc|vitest|jest|vite)(?:\s+[\w./\\="'*?:,@-]+)*$/i,
    /^(?:python|py)\s+-m\s+(pytest|compileall|py_compile)(?:\s+[\w./\\="'*?:,@-]+)*$/i,
    /^(?:python|py)\s+[\w./\\-]+\.py(?:\s+[\w./\\="'-]+)*$/i,
    /^go\s+(test|vet|build)(?:\s+[\w./\\*?-]+)*$/i,
    /^cargo\s+(check|test|build)(?:\s+--[\w-]+)*$/i,
    /^dotnet\s+(test|build)(?:\s+[\w./\\-]+)*(?:\s+--[\w-]+)*$/i,
    /^git\s+(status|diff|log|branch|rev-parse)(?:\s+[\w./\\="'-]+)*$/i,
    /^git\s+switch\s+-c\s+[\w./-]+$/i,
    /^git\s+init$/i,
    /^git\s+add\s+(?:[\w./\\-]+\s*)+$/i,
    /^git\s+commit\s+-m\s+["'][^"'`;&|<>]{1,160}["']$/i,
    /^git\s+push\s+origin\s+[\w./-]+$/i,
    /^rg(?:\s+[\w./\\="'*?:-]+)+$/i
  ];
  if (!allowed.some((pattern) => pattern.test(value))) {
    throw new Error('Esse comando não está na lista segura da Code Sofia.');
  }
  return value;
}

async function runSafeCommand(command, projectId = 'main', onProgress = null) {
  const safeCommand = validateCommand(command);
  const selectedRoot = getSelectedProjectRoot(projectId);
  const report = (message, kind = 'output') => {
    if (typeof onProgress !== 'function' || !message) return;
    try {
      onProgress(String(message).slice(0, 4000), kind);
    } catch {
      // A exibição de progresso nunca pode interromper o comando.
    }
  };
  ...
  return new Promise((resolve) => {
    ...
    const appendOutput = (chunk, kind = 'output') => {
      const text = sanitizeCommandOutput(chunk);
      if (!text) return;
    ...
    const finish = (ok, extra = '') => {
      ...
      settled = true;
      clearTimeout(timer);
      if (extra) appendOutput(extra, 'error');
      report(ok ? '✓ Comando concluído.' : '✗ O comando terminou com erro.', ok ? 'success' : 'error');
      resolve({
        ok,
        command: safeCommand,
        output: output.trim().slice(0, 20000)
      });
    };

    child.stdout?.on('data', (chunk) => appendOutput(chunk, 'output'));
    child.stderr?.on('data', (chunk) => appendOutput(chunk, 'error'));
    child.on('error', (error) => finish(false, error.message || 'Não foi possível iniciar o comando.'));
    child.on('close', (code) => {
      finish(
        code === 0 && !timedOut,
        timedOut ? `\nTempo máximo de ${Math.round(commandTimeout() / 1000)} segundos atingido.` : ''
      );
    });
...
function focusedValidationCommands(changedFiles, projectId = 'main', existingCommands = []) {
  ...
  for (const file of [...new Set(changedFiles || [])]) {
    if (
      ...
      commands.push(`python -m py_compile ${file}`);
  ...
  if (
    !commands.includes('cargo check') &&
    fs.existsSync(path.join(selectedRoot, 'Cargo.toml'))
  ) {
    commands.push('cargo check');
  }
  if (
    !commands.includes('go test ./...') &&
    fs.existsSync(path.join(selectedRoot, 'go.mod'))
  ) {
    commands.push('go test ./...');
  }
...
async function applyCodePlan(planId, onProgress = null) {
  cleanupPlans();
  const plan = getPendingPlan(planId);
  if (!plan) throw new Error('Esse plano expirou. Peça novamente à Code Sofia.');
  ...
  getSelectedProjectRoot(projectId);
  const report = (message, kind = 'stage') => {
    if (typeof onProgress !== 'function') return;
    try {
      onProgress(message, kind);
    } catch {
      // O acompanhamento visual não interfere na aplicação.
    }
  };

  try {
    report('Preparando alterações com backup seguro...');
    for (const action of plan.actions) {
      if (!['write', 'delete', 'move'].includes(action.type)) continue;

      if (action.type === 'move') {
        report(`Movendo arquivo: ${action.from} → ${action.to}`);
        const sourcePath = resolveSafePath(action.from, projectId);
        const destinationPath = resolveSafePath(action.to, projectId);
        for (const [relative, filePath] of [
          [action.from, sourcePath],
          [action.to, destinationPath]
        ]) {
          if (!backedUpPaths.has(relative)) {
            backups.push(createBackup(filePath, actionId, projectId));
            backedUpPaths.add(relative);
          }
        }
        if (!fs.existsSync(sourcePath) || !fs.statSync(sourcePath).isFile()) {
          throw new Error(`Arquivo de origem não encontrado: ${action.from}`);
        }
        if (fs.existsSync(destinationPath)) {
          throw new Error(`O destino já existe: ${action.to}`);
        }
        fs.mkdirSync(path.dirname(destinationPath), { recursive: true });
        fs.renameSync(sourcePath, destinationPath);
        invalidateProjectCache(projectId);
        changedFiles.push(action.from, action.to);
        continue;
      }

      report(`${action.type === 'delete' ? 'Apagando' : 'Atualizando'} arquivo: ${action.path}`);
      const filePath = resolveSafePath(action.path, projectId);
      ...
      if (action.type === 'delete') {
        if (!fs.existsSync(filePath) || !fs.statSync(filePath).isFile()) {
          throw new Error(`Arquivo para apagar não encontrado: ${action.path}`);
        }
    ...
    report(`Falha ao alterar arquivos: ${error.message}`, 'error');
  ...
  if (automaticValidation.length) report('Iniciando verificações automáticas...');
  for (const command of automaticValidation) {
    commandResults.push(await runSafeCommand(command, projectId, report));
  }
  ...
  const result = {
    ...
    canContinue: Boolean(session?.autoMode && session.round < MAX_AGENT_ROUNDS)
  };
  report(
    result.ok ? '✓ Todas as etapas terminaram.' : '✗ Uma ou mais verificações falharam.',
...
async function continueCodeSession(sessionId) {
  ...
  if (session.round >= MAX_AGENT_ROUNDS) {
    return {
      ok: true,
      planId: null,
      sessionId: session.id,
      sessionComplete: true,
      summary: 'A Code Sofia concluiu o limite seguro de etapas. Revise o projeto antes de continuar.',
      actions: [],
      preview: []
    };
  }
...
function getWorkspaceStatus(projectId = 'main') {
  ...
  return {
    ok: true,
    projectId: selectedProjectId,
    name: selectedProjectId === 'main' ? 'Sofia principal' : path.basename(selectedRoot),
