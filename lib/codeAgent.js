...
function buildPrompt(task, contextFiles, extraContext = '') {
  const sections = [];
...
async function createCodePlan(task, contextFiles = []) {
  const cleanTask = String(task || '').trim();
  if (!cleanTask) throw new Error('Conte para a Code Alya o que você quer criar.');

  let response = await askModel(buildPrompt(cleanTask, contextFiles));
