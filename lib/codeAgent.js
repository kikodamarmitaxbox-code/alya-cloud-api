...
function createChangePreview(action) {
  const filePath = resolveSafePath(action.path);
...
function createBackup(filePath, sessionId = crypto.randomUUID()) {
  const relative = relativePath(filePath);
  const destination = path.join(backupRoot, sessionId, relative);
...
function restoreBackups(backups) {
  ...
  for (const snapshot of [...(backups || [])].reverse()) {
    const target = resolveSafePath(snapshot.path);
  ...
  try {
    for (const action of plan.actions) {
      ...
      if (!backedUpPaths.has(action.path)) {
        backups.push(createBackup(filePath, actionId));
    ...
    if (action.type === 'command') {
      if (/^npm\s+install\b/i.test(action.command)) {
        for (const dependencyFile of ['package.json', 'package-lock.json']) {
          const dependencyPath = resolveSafePath(dependencyFile);
          if (fs.existsSync(dependencyPath) && !backedUpPaths.has(dependencyFile)) {
            backups.push(createBackup(dependencyPath, actionId));
      ...
      if (/^npm\s+install\b/i.test(action.command)) {
        changedFiles.push(...['package.json', 'package-lock.json'].filter((dependencyFile) => {
          ...
          if (!snapshot || !fs.existsSync(resolveSafePath(dependencyFile))) return false;
          const before = snapshot.existed && fs.existsSync(path.resolve(projectRoot, snapshot.backup))
            ...
            : '';
          const after = fs.readFileSync(resolveSafePath(dependencyFile), 'utf8');
          return before !== after;
        }));
      }
    }
...
function saveProjectFile(requestedPath, content) {
  ...
  const actionId = crypto.randomUUID();
  const backup = createBackup(filePath, actionId);
  fs.mkdirSync(path.dirname(filePath), { recursive: true });
  try {
...
function getActionHistory(limit = 12) {
...
function undoCodeAction(actionId = '') {
  ...
  const restoredFiles = restoreBackups(entry.backups);
...
function getWorkspaceStatus() {
  return {
    ...
    canUndo: getActionHistory(1).some((entry) => entry.canUndo)
  };
}
