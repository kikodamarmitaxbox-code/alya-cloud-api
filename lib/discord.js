...
class DiscordManager {
  constructor(options = {}) {
    ...
    if (!this.privateOwnerIds.length && !this.privateOwnerUsernames.length) {
      this.privateOwnerUsernames = [...this.deleteUsernames];
    }
  ...
  async sendScreenshot(alias, buffer, requestedBy = 'Alya') {
    if (!this.client || !this.ready) {
    ...
    await channel.send({
      content: `📸 Captura solicitada por ${String(requestedBy || 'Alya').slice(0, 64)}.`,
      files: [{
        attachment: buffer,
