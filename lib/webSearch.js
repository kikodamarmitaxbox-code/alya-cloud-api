...
async function searchWeb(query) {
  ...
  logger.info('Iniciando pesquisa na web ao vivo.', { queryLength: cleanQuery.length });
...
function shouldSearchWeb(text) {
  ...
  const freshnessSignal = /\b(hoje|agora|atual|atualmente|recente|última|ultima|mais recente|em tempo real)\b/i.test(q);
  const liveInformation = /\b(notícias?|cotação|preço|valor do dólar|clima|previsão do tempo|placar|resultado da partida)\b/i.test(q);
  const sportsResult = /\b(ganhou|venceu|campeão|campeao|resultado)\b/i.test(q)
    && /\b(jogo|partida|campeonato|copa|champions|libertadores|brasileirão|brasileirao|ufc|valorant|csgo|dota|lol)\b/i.test(q);

  return liveInformation || sportsResult || freshnessSignal;
}
